...
async function problemA () {
  ...
  // AsyncAwait version
  const response = await Promise.all([promisifiedReadFile('poem-two/stanza-01.txt'), promisifiedReadFile('poem-two/stanza-02.txt')])
...
async function problemB () {
  ...
  // AsyncAwait version
  const promises = filenames.map(f => promisifiedReadFile(f));
